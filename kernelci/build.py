...
def _run_make(kdir, arch, target=None, jopt=None, silent=True, cc='gcc',
    ...
    if log_file:
        ...
        make_cmd += " | tee -a {}".format(os.path.join(output, log_file))
    cmd = "cd {} && {}".format(kdir, make_cmd)
    return shell_cmd(cmd, True)


def _make_defconfig(defconfig, kwargs):
...
""".format(kdir=kdir, output=output, base=os.path.join(output, '.config'),
...
def build_kernel(build_env, kdir, arch, defconfig=None, jopt=None,
    ...
    # ToDo: check how to use modified env without propagating to parent process
    #env = os.environ.copy()
    os.environ['KBUILD_BUILD_USER'] = 'KernelCI'

    start_time = time.time()
    if defconfig:
        result = _make_defconfig(defconfig, kwargs)
    ...
    if mods and mod_path:
        ...
        _run_make(target='modules_install', opts=opts, **kwargs)
...
def install_kernel(kdir, arch, defconfig, tree_name, tree_url, git_branch,
    ...
    for root, _, files in os.walk(dts_dir):
        for f in fnmatch.filter(files, '*.dtb'):
            ...
            dtb_dir = dtb_path.split(dts_dir)[1].lstrip(os.path.sep)
            dest_path = os.path.join(dtbs, dtb_dir)
    ...
    publish_path = '/'.join([
        tree_name, git_branch, describe, arch, defconfig, build_env,
    ])
...
def push_kernel(kdir, api, token, install='_install_'):
    ...
    for root, _, files in os.walk(install_path):
        for f in files:
            px = root.split(install_path)[1].lstrip('/')
            artifacts[os.path.join(px, f)] = os.path.join(root, f)
